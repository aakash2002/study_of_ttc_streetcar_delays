...
## Overview of dataset
The following data was made available from OpenData Toronto (@opendatatoronto) for the year 2023. The data was extracted from the server and saved locally using in a raw data folder using the instructions provided by the documentation. We note that this was the only available dataset with frequent updates and data dating back to 2014. Some of the features part of this dataset are date, time, incident, min delay, bound and location.
